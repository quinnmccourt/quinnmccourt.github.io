...
_.contains = (arr, val) => (arr.indexOf(val) === -1) ?  false : true;
...
_.unique = arr => arr.filter((item, index) => arr.indexOf(item) >= index);
...
_.filter = (arr, func) => {
    const retArr = [];
  _.each(arr, (ele, i, arr) => {
     if (func(ele, i, arr)) retArr.push(arr[i]);
  });
  return retArr;
};
...
*/
_.reject = (arr, func) => {
    return _.filter(arr, (ele, i, arr) => !func(ele, i, arr));
};
...
_.partition = (arr, func) => {
    const retArr = [];
   retArr.push(_.filter(arr, func));
   retArr.push(_.reject(arr, func));
    return retArr;
};
...
_.map = (col, func) => {
    const retArr = [];
    _.each(col, (ele, key, col) => {
        retArr.push(func(ele, key, col));
    });
